...
from ui.base_ui import BaseUI  # Replace the old UI import
...
class InspectionSystem:
    ...
    status_callback=self.ui.status.add_message
)
    ...
    def process_piece(self):
        ...
        try:
            self.ui.status.add_message("=== Starting piece processing ===")
            ...
            self.ui.status.add_message(f"Processing error: {str(e)}", is_alert=True)

        finally:
            self.analysis_in_progress = False
            self.ui.status.current_status['state'] = 'normal'
            self.ui.status.current_status['alert'] = None
    ...
    def run_state_machine(self):
        ...
        try:
            ...
            self.ui.status.add_message(f"State machine error: {str(e)}", is_alert=True)
    ...
    def _handle_piece_processing(self, sensor1_state: bool):
        ...
        if self.ui.status.current_status['state'] == 'normal':
            if sensor1_state and not self.piece_in_progress:
                self.piece_in_progress = True
                self.ui.status.add_message("Piece detected")
                self.waiting_for_sensor2 = False
                self.ui.status.current_status['alert'] = None
                ...
                if not sensor1_state:
                    ...
                    if self.waiting_for_sensor2 and (time.time() - self.sensor2_check_time) > SENSOR2_WAIT_TIME:
                        if not self.gpio.read_sensor2():
                            self.ui.status.current_status['alert'] = "No piece fell in slot!"
                            self.ui.status.current_status['state'] = 'error_recovery'
                        else:
        ...
        elif self.ui.status.current_status['state'] == 'error_recovery':
            if sensor1_state:
                self.ui.status.current_status['state'] = 'normal'
                self.piece_in_progress = True
                self.waiting_for_sensor2 = False
    ...
    def run(self):
        ...
        try:
            ...
            self.ui.status.add_message("Shutdown requested...")
        except Exception as e:
            self.ui.status.add_message(f"Critical error: {str(e)}", is_alert=True)
        finally:
            self.cleanup()
